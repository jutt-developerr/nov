        ...
        asort($firebase_drivers);
            ...
            if (!empty($firebase_drivers)) {
                ...
                foreach ($firebase_drivers as $key => $firebase_driver) {
                    ...
                    DB::enableQueryLog();
                    $nearest_drivers = Driver::where('active', 1)->where('approve', 1)->where('available', 1)->whereIn('id', $nearest_driver_ids)->whereNotIn('id', $meta_drivers)->whereNotIn('id',$rejected_drivers)->orderByRaw(DB::raw("FIELD(id, " . implode(',', $nearest_driver_ids) . ")"))->limit(10)->get();
